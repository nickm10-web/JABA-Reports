...
const outputPath = path.join(dataDir, 'michigan-athlete-overview.json');
...
// In feb_17, athlete.school is a string (not object)
const posts = allPosts.filter(p => {
  const sch = p?.athlete?.school;
  const name = typeof sch === 'string' ? sch : (sch?.name || '');
  return name.toLowerCase() === 'michigan';
});
console.log(`Michigan posts: ${posts.length}`);

// Build follower map from roster (schoolName field)
const normalize = (value) => String(value || '').toLowerCase().replace(/[^a-z0-9]/g, '');
const rosterFollowerMap = new Map();
for (const athlete of rosterRows) {
  const schoolName = athlete?.schoolName || '';
  if (!schoolName.toLowerCase().includes('michigan') || schoolName.toLowerCase().includes('state')) continue;
...
// Total followers from roster (Michigan hardcoded from Big 10 benchmarks)
const totalFollowers = 2381406;
...
for (const [name, sig] of [['Logo', logo], ['Mention', mention], ['Collab', collaboration]]) {
  console.log(`${name}: ${sig.posts} posts (${(sig.posts / posts.length * 100).toFixed(1)}%), avgLikes: ${Math.round(sig.likes)}, engRate: ${(sig.engagementRate * 100).toFixed(4)}%, lift: ${sig.delta.toFixed(1)}%, EMV: $${Math.round(sig.emv).toLocaleString()}`);
}

// === PARTNERSHIPS ===
const excludeBrands = new Set([
  '@on3recruits', '@rivalsdotcom', '@247sports',
  '@umichathletics', '@umichfootball', '@michiganwolverines',
...
for (const p of partnerships.slice(0, 20)) {
  console.log(`  ${p.brand}: ${p.posts} posts, avgLikes: ${Math.round(p.avgLikes)}, lift: ${p.liftMultiplier}, EMV: $${Math.round(p.emv).toLocaleString()}`);
}
...
tsx += `// totalFollowers: ${totalFollowers}\n`;
tsx += `const ipData = {\n`;
...
fs.writeFileSync('/tmp/michigan_tsx_data.txt', tsx);
console.log('\nTSX data written to /tmp/michigan_tsx_data.txt');
...
const output = {
  ...
  totalEmv: Number(totalEmv.toFixed(2)),
  collaboration,
  logo,
  mention,
  counters: {
