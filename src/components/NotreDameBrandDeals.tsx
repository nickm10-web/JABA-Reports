...
const NOTRE_DAME_SHORT_NAME = 'Notre Dame';
const NOTRE_DAME_SCHOOL_NAME = 'University of Notre Dame';
const NOTRE_DAME_CONFERENCE = 'Independent / ACC';
const SPONSORED_CONTENT_EXCLUSIONS = new Set([
  'notredamethreads', 'ndmbb', 'ndwbb', 'ndalumni', 'myplayersoftball', 'hammesnotredame',
  'theirishscoop', 'ig2bpodcast', 'cfbalerts', 'on3recruits', 'playerstribune', 'pbtf_events',
  'bradleyrobidouxvideo', 'mingtsai', 'dwarnett', 'leonardmoore', 'eliraridon', 'spencesummeruns',
  'kaia.cortes', 'iris.arlo',
]);

function deriveTeamContentFromNotreDameTeamData(
  teams: Array<Record<string, unknown>>,
  teamPosts: Array<Record<string, unknown>>
): UCLATeamContent | null {
  if (!teams.length) return null;

  const teamAccounts: UCLATeamAccount[] = teams.map((team) => {
    const metrics = (team.metrics as Record<string, unknown> | undefined) ?? {};
    const ninetyDays = (metrics.ninetyDays as Record<string, unknown> | undefined) ?? {};
    const allTime = (metrics.allTime as Record<string, unknown> | undefined) ?? {};

    const followers = Number(ninetyDays.followers ?? allTime.followers ?? 0);
    const likes90d = Number(ninetyDays.likes ?? 0);
    const comments90d = Number(ninetyDays.comments ?? 0);
    const contentCount90d = Number(ninetyDays.contentCount ?? 0);
    const followersGrowth90d = Number(ninetyDays.followersGrowth ?? 0);
    const logoContentCount = Number(ninetyDays.logoContentCount ?? 0);
    const engagementRate = Number(ninetyDays.engagementRate ?? 0);
    const engagementRateLogoLift = Number(ninetyDays.avgEngagementRateWithLogo ?? 0);

    return {
      sport: String(team.sport ?? 'UNKNOWN'),
      profilePicture: team.profilePicture ? String(team.profilePicture) : undefined,
      conference: team.conferenceName ? String(team.conferenceName) : undefined,
      followers,
      engagementRate,
      likes90d,
      comments90d,
      ...
      followersGrowth90d,
      logoContentCount,
      engagementRateLogoLift,
      contentLikes: likes90d,
      contentComments: comments90d,
      contentPosts: contentCount90d,
    };
  });

  const topPosts: UCLATeamPost[] = teamPosts
    .map((post) => {
      const metrics = (post.metrics as Record<string, unknown> | undefined) ?? {};
      const team = (post.team as Record<string, unknown> | undefined) ?? {};
      const publishedAtRaw = post.publishedAt as string | { $date?: string } | undefined;
      const publishedAt = typeof publishedAtRaw === 'string' ? publishedAtRaw : publishedAtRaw?.$date;
      ...
      return {
        ...
        caption: post.caption ? String(post.caption) : undefined,
        url: post.url ? String(post.url) : undefined,
        permalink: post.permalink ? String(post.permalink) : undefined,
        publishedAt,
  ...
  const totalFollowers = teamAccounts.reduce((sum, account) => sum + account.followers, 0);
  const totalTeamPosts = teamAccounts.reduce((sum, account) => sum + (account.contentPosts || 0), 0);
  const totalContentLikes = teamAccounts.reduce((sum, account) => sum + (account.contentLikes || 0), 0);
  const totalContentComments = teamAccounts.reduce((sum, account) => sum + (account.contentComments || 0), 0);

  return {
    school: { name: NOTRE_DAME_SCHOOL_NAME },
    totalTeamAccounts: teamAccounts.length,
    totalFollowers,
    totalTeamPosts,
    totalContentLikes,
    totalContentComments,
    teamAccounts,
    topPosts,
  };
}

function mergeNotreDameAthleteFollowers(
  roster: UCLARoster | null,
  backupRosterRows: Array<Record<string, unknown>>
): UCLARoster | null {
  if (!roster?.athletes?.length || !backupRosterRows.length) return roster;

  const followersByName = new Map<string, number>();
  for (const row of backupRosterRows) {
    ...
    const allTime = (metrics.allTime as Record<string, unknown> | undefined) ?? {};
    const followers = Number(ninetyDays.followers ?? allTime.followers ?? 0);
    if (followers > 0) followersByName.set(fullName, followers);
  }

  if (followersByName.size === 0) return roster;

  return {
    ...roster,
    athletes: roster.athletes.map((athlete) => {
      const key = (athlete.name || '').trim().toLowerCase();
      const fallbackFollowers = followersByName.get(key);
      return {
        ...athlete,
        followers: athlete.followers && athlete.followers > 0
          ? athlete.followers
          : (fallbackFollowers ?? 0),
      };
    }),
...
function buildNotreDameBenchmarksFromSource(
  ipImpact: UCLAIPImpact | null,
  roster: UCLARoster | null,
  posts: UCLAAthletePost[],
  networkBenchmarks: BenchmarksData | null
): BenchmarksData | null {
  if (!ipImpact) return null;

  const totalContents = ipImpact.overall.totalContents || posts.length;
  const totalLikes = ipImpact.overall.totalLikes || posts.reduce((sum, post) => sum + (post.metrics?.likes || 0), 0);
  const totalComments = ipImpact.overall.totalComments || posts.reduce((sum, post) => sum + (post.metrics?.comments || 0), 0);
  const partnerCount = new Set(posts.map((post) => (post.sponsorPartner || '').trim().toLowerCase()).filter(Boolean)).size;
  const allCollabPosts = posts.filter((post) => !!(post as UCLAAthletePost & { isCollaboration?: boolean }).isCollaboration).length;

  const school: BenchmarkSchool = {
    school: NOTRE_DAME_SCHOOL_NAME,
    shortName: NOTRE_DAME_SHORT_NAME,
    conference: NOTRE_DAME_CONFERENCE,
    isBigTen: false,
    followers: ipImpact.followers || 0,
    totalContents,
    totalLikes,
    totalComments,
    engagementRate: ipImpact.overall.engagementRate || 0,
    emv: ipImpact.overall.emv || estimateEmv(totalLikes, totalComments),
    collabLift: ipImpact.collaboration.avgLift || 0,
    logoLift: ipImpact.logo.avgLift || 0,
    captionLift: ipImpact.orgInCaption.avgLift || 0,
    collabPosts: allCollabPosts,
    logoPosts: ipImpact.counts.hasOrganizationLogo || 0,
    withIpCount: ipImpact.counts.withIp || 0,
    likesPerPost: totalContents ? totalLikes / totalContents : 0,
    commentsPerPost: totalContents ? totalComments / totalContents : 0,
    ipPercentage: totalContents ? ((ipImpact.counts.withIp || 0) / totalContents) * 100 : 0,
    partnerCount,
    athleteCount: roster?.totalAthletes || roster?.athletes?.length || 0,
    sponsoredPosts: posts.filter((post) => !!post.sponsorPartner).length,
  };

  const schools = [
    ...(networkBenchmarks?.schools ?? []).filter((s) => s.shortName !== NOTRE_DAME_SHORT_NAME),
    school,
  ];

  const computeAverage = (rows: BenchmarkSchool[]): BenchmarkAvg => {
    if (rows.length === 0) {
      return {
        totalContents: 0, totalLikes: 0, totalComments: 0, emv: 0, collabLift: 0, logoLift: 0,
        collabPosts: 0, logoPosts: 0, withIpCount: 0, likesPerPost: 0, commentsPerPost: 0,
        ipPercentage: 0, partnerCount: 0, athleteCount: 0, sponsoredPosts: 0,
      };
    }
    const sum = rows.reduce((acc, row) => ({
      totalContents: acc.totalContents + (row.totalContents || 0),
      totalLikes: acc.totalLikes + (row.totalLikes || 0),
      totalComments: acc.totalComments + (row.totalComments || 0),
      emv: acc.emv + (row.emv || 0),
      collabLift: acc.collabLift + (row.collabLift || 0),
      logoLift: acc.logoLift + (row.logoLift || 0),
      collabPosts: acc.collabPosts + (row.collabPosts || 0),
      logoPosts: acc.logoPosts + (row.logoPosts || 0),
      withIpCount: acc.withIpCount + (row.withIpCount || 0),
      likesPerPost: acc.likesPerPost + (row.likesPerPost || 0),
      commentsPerPost: acc.commentsPerPost + (row.commentsPerPost || 0),
      ipPercentage: acc.ipPercentage + (row.ipPercentage || 0),
      partnerCount: acc.partnerCount + (row.partnerCount || 0),
      athleteCount: (acc.athleteCount || 0) + (row.athleteCount || 0),
      sponsoredPosts: (acc.sponsoredPosts || 0) + (row.sponsoredPosts || 0),
    }), {
      totalContents: 0, totalLikes: 0, totalComments: 0, emv: 0, collabLift: 0, logoLift: 0,
      collabPosts: 0, logoPosts: 0, withIpCount: 0, likesPerPost: 0, commentsPerPost: 0,
      ipPercentage: 0, partnerCount: 0, athleteCount: 0, sponsoredPosts: 0,
    ...
    const n = rows.length;
    return {
      totalContents: sum.totalContents / n,
      totalLikes: sum.totalLikes / n,
      totalComments: sum.totalComments / n,
      emv: sum.emv / n,
      collabLift: sum.collabLift / n,
      logoLift: sum.logoLift / n,
      collabPosts: sum.collabPosts / n,
      logoPosts: sum.logoPosts / n,
      withIpCount: sum.withIpCount / n,
      likesPerPost: sum.likesPerPost / n,
      commentsPerPost: sum.commentsPerPost / n,
      ipPercentage: sum.ipPercentage / n,
      partnerCount: sum.partnerCount / n,
      athleteCount: (sum.athleteCount || 0) / n,
      sponsoredPosts: (sum.sponsoredPosts || 0) / n,
    };
  };

  const rankFor = (rows: BenchmarkSchool[], key: keyof BenchmarkSchool) => {
    const sorted = [...rows].sort((a, b) => (Number(b[key] || 0) - Number(a[key] || 0)));
    const idx = sorted.findIndex((row) => row.shortName === NOTRE_DAME_SHORT_NAME);
    return { rank: idx >= 0 ? idx + 1 : rows.length, of: rows.length };
  };

  const metricKeys: Array<keyof BenchmarkSchool> = [
    'totalContents', 'totalLikes', 'totalComments', 'likesPerPost', 'collabPosts', 'ipPercentage',
  ];
  const bigTenScope = schools.filter((s) => s.isBigTen);
  const rankAll = Object.fromEntries(metricKeys.map((key) => [key, rankFor(schools, key)]));
  const rankBigTen = Object.fromEntries(metricKeys.map((key) => [key, rankFor(bigTenScope.length > 0 ? [...bigTenScope, school] : [school], key)]));

  return {
    schools,
    bigTenAvg: computeAverage(bigTenScope),
    allSchoolAvg: computeAverage(schools),
    uclaRanks: rankAll as BenchmarksData['uclaRanks'],
    uclaBigTenRanks: rankBigTen as BenchmarksData['uclaBigTenRanks'],
  };
}

function buildNotreDameTeamBenchmarksFromSource(
  teamContent: UCLATeamContent | null,
  networkTeamBenchmarks: TeamBenchmarksData | null
): TeamBenchmarksData | null {
  if (!teamContent) return null;

  const engagementRates = teamContent.teamAccounts
    .map((team) => (team.engagementRate || 0) * 100)
    .filter((value) => value > 0);
  const avgEngagementRate = engagementRates.length
    ? engagementRates.reduce((sum, value) => sum + value, 0) / engagementRates.length
    : 0;

  const ndTeamSchool: TeamBenchmarkSchool = {
      school: NOTRE_DAME_SCHOOL_NAME,
      shortName: NOTRE_DAME_SHORT_NAME,
      conference: NOTRE_DAME_CONFERENCE,
      isBigTen: false,
      teamAccounts: teamContent.totalTeamAccounts,
      totalFollowers: teamContent.totalFollowers,
      totalLikes: teamContent.totalContentLikes,
      totalPosts: teamContent.totalTeamPosts,
      avgEngagementRate,
      likesPerPost: teamContent.totalTeamPosts ? teamContent.totalContentLikes / teamContent.totalTeamPosts : 0,
    };

  return {
    schools: [
      ...((networkTeamBenchmarks?.schools ?? []).filter((s) => s.shortName !== NOTRE_DAME_SHORT_NAME)),
      ndTeamSchool,
    ],
  };
}

export function NotreDameBrandDeals({ onBack }: { onBack?: () => void }) {
  ...
  const activeTabs = useMemo(
    () => (teamContent ? TABS : TABS.filter((tab) => tab.id !== 'teams' && tab.id !== 'content')),
    [teamContent]
  );
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [ipData, rosterData, contentsData, teamsData, teamContentsData, benchData, teamBenchData, networkBenchData, networkTeamBenchData, backupRosterData] = await Promise.allSettled([
          fetchJson<UCLAIPImpact>('/data/notre-dame-ip-impact.json'),
          fetchJson<UCLARoster>('/data/notre-dame-roster.json'),
          fetchJson<UCLAAthletePost[]>('/data/NotreContents.json'),
          fetchJson<Array<Record<string, unknown>>>('/data/NotreTeams.json'),
          fetchJson<Array<Record<string, unknown>>>('/data/NotreTeamContens.json'),
          fetchJson<BenchmarksData>('/data/notre-dame-benchmarks.json'),
          fetchJson<TeamBenchmarksData>('/data/notre-dame-team-benchmarks.json'),
          fetchJson<BenchmarksData>('/data/ucla-benchmarks.json'),
          fetchJson<TeamBenchmarksData>('/data/ucla-team-benchmarks.json'),
          fetchJson<Array<Record<string, unknown>>>('/data/NotreRoster.json'),
        ]);

        const resolvedIpImpact = ipData.status === 'fulfilled' && ipData.value ? ipData.value : null;
        const resolvedRoster = rosterData.status === 'fulfilled' && rosterData.value ? rosterData.value : null;
        const resolvedPosts = contentsData.status === 'fulfilled' && Array.isArray(contentsData.value) ? contentsData.value : [];
        const resolvedTeams = teamsData.status === 'fulfilled' && Array.isArray(teamsData.value) ? teamsData.value : [];
        const resolvedTeamContents = teamContentsData.status === 'fulfilled' && Array.isArray(teamContentsData.value) ? teamContentsData.value : [];
        const resolvedBackupRoster = backupRosterData.status === 'fulfilled' && Array.isArray(backupRosterData.value) ? backupRosterData.value : [];
        const hydratedRoster = mergeNotreDameAthleteFollowers(resolvedRoster, resolvedBackupRoster);
        const resolvedNetworkBenchmarks = networkBenchData.status === 'fulfilled' ? networkBenchData.value : null;
        const resolvedNetworkTeamBenchmarks = networkTeamBenchData.status === 'fulfilled' ? networkTeamBenchData.value : null;
        const derivedTeamContent = deriveTeamContentFromNotreDameTeamData(resolvedTeams, resolvedTeamContents);

        setRoster(hydratedRoster);
        setAthletePosts(resolvedPosts);
        setSponsoredPosts(resolvedPosts.filter((post) => !!post.sponsorPartner));
        setTeamContent(derivedTeamContent);
        const displayIpImpact = buildIpImpactForDisplay(resolvedIpImpact, resolvedPosts);
        if (displayIpImpact) setIpImpact(displayIpImpact);

        setBenchmarks(
          benchData.status === 'fulfilled' && benchData.value
            ? benchData.value
            : buildNotreDameBenchmarksFromSource(displayIpImpact, hydratedRoster, resolvedPosts, resolvedNetworkBenchmarks)
        );
        setTeamBenchmarks(
          teamBenchData.status === 'fulfilled' && teamBenchData.value
            ? teamBenchData.value
            : buildNotreDameTeamBenchmarksFromSource(derivedTeamContent, resolvedNetworkTeamBenchmarks)
        );
      } catch (err) {
        console.error('Failed to load Notre Dame data', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (!activeTabs.some((tab) => tab.id === activeTab)) {
  ...
  const athleteFollowers = useMemo(() => {
    if (!roster?.athletes?.length) return null;
    const explicitTotal = (roster as (UCLARoster & { totalFollowers?: number }) | null)?.totalFollowers;
    if (typeof explicitTotal === 'number' && explicitTotal > 0) return explicitTotal;
    const total = roster.athletes.reduce((sum, athlete) => sum + (athlete.followers || 0), 0);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F7F9FC]">
        <div className="text-gray-700 text-lg">Loading Notre Dame Report…</div>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <div className="text-lg font-semibold text-[#0F1D2E]">Notre Dame Athlete + Team Performance Report</div>
          ...
          {activeTab === 'benchmarks' && (
            benchmarks ? (
              ...
              <GlassPanel className="p-8">
                <h2 style={headerStyle} className="text-lg font-bold uppercase tracking-tight text-[#0F1D2E]">Benchmarks Unavailable</h2>
                <p className="text-sm text-[#5B6B82] mt-2">Notre Dame is currently configured without a benchmark comparison file.</p>
              </GlassPanel>
            )
          )}
    ...
    <div className="space-y-10">
      <GlassPanel className="p-6 md:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1.3fr_0.7fr] gap-6">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-[#2774AE]">NOTRE DAME SOCIAL PERFORMANCE</p>
      ...
      <GlassPanel className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl border border-[#E1E7F0] bg-white p-4">
            ...
            <p className="text-sm text-[#5B6B82]">Across {formatNumber(roster?.totalAthletes ?? athleteList.length)} athletes</p>
          </div>
          <div className="rounded-xl border border-[#E1E7F0] bg-white p-4">
...
function ContentTab({ teamContent, athletePosts }: { teamContent: UCLATeamContent; athletePosts: UCLAAthletePost[] }) {
  ...
  const normalizedAthletePosts = useMemo(() => {
    return athletePosts.map(p => ({
      ...
      sponsorHandle: String(p.sponsorPartner || '').replace(/^@/, '').toLowerCase().trim(),
      caption: p.caption,
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (sponsoredOnly) {
      list = list.filter(
        p => p.isSponsored && !SPONSORED_CONTENT_EXCLUSIONS.has(p.sponsorHandle)
      );
    }
    return list;
      ...
      <GlassPanel className="p-6 bg-[#F3F6FB]">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div>
              <h2 style={headerStyle} className="text-lg font-bold uppercase tracking-tight text-[#0F1D2E]">Notre Dame's Highest Performing Content</h2>
              <p className="text-sm text-[#5B6B82]">Top posts across Notre Dame team + athlete accounts</p>
            </div>
...
function SponsoredTab({ posts }: { posts: UCLASponsoredPost[] }) {
  ...
  const NON_BRAND_EXCLUSIONS = new Set([
    ...
    // Notre Dame cleanup: team/school pages, podcasts/media, and individual creators
    'notredamethreads','ndmbb','ndwbb','ndalumni','myplayersoftball',
    'hammesnotredame','theirishscoop','ig2bpodcast','cfbalerts','on3recruits',
    'playerstribune','pbtf_events','bradleyrobidouxvideo',
    'mingtsai','dwarnett','leonardmoore','eliraridon','spencesummeruns',
    'kaia.cortes','iris.arlo',
  ]);
  ...
  const BRAND_ALIASES: Record<string, string> = {
    ...
    // Notre Dame cleanup: merge known duplicate brand variants
    'c4enrgy': 'c4energy',
    'powerade_us': 'powerade',
    'drscholls_usa': 'drscholls',
  };
  ...
  filtered.forEach(p => {
    ...
    if (existing) {
      ...
      if (p.athlete?.name) existing.athleteNames.add(p.athlete.name);
      ...
      brandMap.set(key, {
        ...
        athleteNames: new Set(p.athlete?.name ? [p.athlete.name] : []),
  ...
  const BRAND_DOMAINS: Record<string, string> = {
    ...
    mcdonaldsgreaterindiana: 'mcdonalds.com',
  };

  const getBrandLogoCandidates = (brand: string) => {
    const clean = brand.replace(/^@/, '').replace(/[^a-z0-9]/gi, '').toLowerCase();
    const domain = BRAND_DOMAINS[clean] || `${clean}.com`;
    const candidates = [
      brandLogoMap[clean],
      `https://logo.clearbit.com/${domain}`,
      `https://logo.dev/${domain}`,
      `https://ui-avatars.com/api/?name=${encodeURIComponent(clean.slice(0, 2).toUpperCase() || 'BR')}&size=96&background=EEF3FA&color=1E2A3B&bold=true`,
  ...
  return (
    <div className="space-y-8">
      <GlassPanel className="p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#5B6B82]">Sponsored posts across Notre Dame athletes</p>
          </div>
        </div>
      </GlassPanel>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {topBrands.map(brand => {
          const { clean, candidates } = getBrandLogoCandidates(brand.brand);
          const visibleAthletes = brand.athleteNames.slice(0, 6);
          const remainingAthletes = Math.max(0, brand.athleteNames.length - visibleAthletes.length);
          return (
                ...
                <div className="w-12 h-12 rounded-xl bg-[#F3F6FB] border border-[#E1E7F0] flex items-center justify-center overflow-hidden">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      const currentIdx = Number(img.dataset.logoIdx || '0');
                      const nextIdx = currentIdx + 1;
                      if (nextIdx < candidates.length) {
                        img.dataset.logoIdx = String(nextIdx);
              ...
              <div className="mt-3">
                ...
                <p className="text-xs text-[#5B6B82] line-clamp-2">
                  {brand.athleteNames.length > 0
                    ? `${visibleAthletes.join(', ')}${remainingAthletes > 0 ? ` +${remainingAthletes} more` : ''}`
                    : 'N/A'}
                </p>
              </div>
                    ...
                    <div key={`${brand.brand}-${idx}`} className="text-xs text-[#5B6B82] flex items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate">
                          {(post.athlete?.name || 'Unknown')} • {formatNumber(post.metrics?.likes || 0)} likes
                        </div>
                        <div className="text-[#7A8AA3]">{formatDate(postDate)}</div>
                      </div>
                    </div>
                  );
                })}
...
function BenchmarksTab({ benchmarks, teamBenchmarks }: { benchmarks: BenchmarksData; teamBenchmarks: TeamBenchmarksData | null }) {
  ...
  const selfSchool = benchmarks.schools.find(s => s.shortName === NOTRE_DAME_SHORT_NAME) ?? benchmarks.schools[0];
  if (!selfSchool) return null;

  const peers = benchmarks.schools.filter(s => s.shortName !== selfSchool.shortName);
  const avg = benchmarks.allSchoolAvg;
  const ranks = benchmarks.uclaRanks;
  ...
  const metricRows: Array<{
    ...
    { label: 'Total Posts', key: 'totalContents', uclaVal: selfSchool.totalContents, avgVal: avg.totalContents, format: formatNumber },
    { label: 'Total Likes', key: 'totalLikes', uclaVal: selfSchool.totalLikes, avgVal: avg.totalLikes, format: formatNumber },
    { label: 'Total Comments', key: 'totalComments', uclaVal: selfSchool.totalComments, avgVal: avg.totalComments, format: formatNumber },
    { label: 'Likes / Post', key: 'likesPerPost', uclaVal: selfSchool.likesPerPost, avgVal: avg.likesPerPost, format: formatNumber },
    { label: 'Collab Posts', key: 'collabPosts', uclaVal: selfSchool.collabPosts, avgVal: avg.collabPosts, format: formatNumber },
    { label: 'IP Posts %', key: 'ipPercentage', uclaVal: selfSchool.ipPercentage, avgVal: avg.ipPercentage, format: (v: number) => formatPercent(v, 1) },
  ];

  const diffPct = (a: number, b: number) => b !== 0 ? ((a - b) / Math.abs(b)) * 100 : 0;

  const teamSchools = teamBenchmarks?.schools ?? [];
    ...
    <div className="space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-[#5B6B82]">
                {dataType === 'athletes'
                  ? `${selfSchool.shortName} athlete NIL posts vs all schools in dataset (${peers.length} schools)`
                  : `Official team social pages — all schools (${teamSchools.length} schools)`
              }
            </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 rounded-full border border-[#E1E7F0] p-1 bg-[#F7F9FC]">
                {([
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${dataType === chip.key ? 'text-[#0F1D2E] shadow-sm' : 'text-[#5B6B82] hover:text-[#1E2A3B]'}`}
                    style={dataType === chip.key ? { backgroundColor: ndGold } : undefined}
                  >
                    {chip.label}
                  </button>
                ))}
              </div>
              <div className="text-xs font-semibold uppercase tracking-[0.2em] text-[#7A8AA3]">All Schools</div>
            </div>
          </div>
        </div>
      </GlassPanel>
      ...
      <GlassPanel className="p-6">
        <h3 style={headerStyle} className="text-base font-bold uppercase tracking-tight mb-4 text-[#0F1D2E]">
          {selfSchool.shortName} vs Network Average
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="text-xs uppercase tracking-[0.2em] text-[#5B6B82]">
                ...
                <th className="text-right py-2 px-3">{selfSchool.shortName}</th>
                <th className="text-right py-2 px-3">All Avg</th>
      ...
      <GlassPanel className="p-6">
        <h3 style={headerStyle} className="text-base font-bold uppercase tracking-tight mb-4 text-[#0F1D2E]">
          Key Metrics vs Network
        </h3>
        <div className="space-y-5">
          {[
            { label: 'Total Likes', uclaVal: selfSchool.totalLikes, avgVal: avg.totalLikes },
            { label: 'Collab Posts', uclaVal: selfSchool.collabPosts, avgVal: avg.collabPosts },
            { label: 'Likes / Post', uclaVal: selfSchool.likesPerPost, avgVal: avg.likesPerPost },
          ].map(item => {
            ...
            return (
              <div key={item.label}>
                <div className="flex items-center justify-between text-sm mb-1">
                  ...
                  <span className="text-xs text-[#7A8AA3]">{selfSchool.shortName} {formatNumber(item.uclaVal)} vs Avg {formatNumber(item.avgVal)}</span>
                </div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs w-10" style={{ color: ndBlue }}>{selfSchool.shortName}</span>
                    <div className="flex-1 h-5 bg-[#F0F4FA] rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${uclaW}%`, backgroundColor: ndBlue }} />
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      <GlassPanel className="p-6">
        <h3 style={headerStyle} className="text-base font-bold uppercase tracking-tight mb-1 text-[#0F1D2E]">
          All Schools Athlete Posts Leaderboard
        </h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
              ...
              <tr className="text-xs uppercase tracking-[0.2em] text-[#5B6B82] bg-[#F3F6FB]">
                ...
                {([
                  ...
                  <th
                    ...
                    style={lbSort === col.key ? { color: ndBlue } : undefined}
            ...
            <tbody>
              {[selfSchool, ...peers].sort((a, b) => b[lbSort] - a[lbSort]).map(s => (
                <tr
                  key={s.shortName}
                  className={`border-t border-[#E1E7F0] ${s.shortName === selfSchool.shortName ? 'font-semibold' : 'hover:bg-[#F3F6FB]'}`}
                  style={s.shortName === selfSchool.shortName ? { backgroundColor: 'rgba(12,35,64,0.06)' } : undefined}
                >
                  <td className="py-3 px-3 text-sm text-[#0F1D2E]">
                    {s.shortName === selfSchool.shortName ? '→ ' : ''}{s.shortName}
                  </td>
      ...
      {dataType === 'teams' && (
        <GlassPanel className="p-6">
          <h3 style={headerStyle} className="text-base font-bold uppercase tracking-tight mb-1 text-[#0F1D2E]">
            All Schools Team Social Pages Leaderboard
          </h3>
              ...
              <table className="w-full">
                  ...
                  <tr className="text-xs uppercase tracking-[0.2em] text-[#5B6B82] bg-[#F3F6FB]">
                    ...
                    {([
                      ...
                      <th
                        key={col.key}
                        className="text-right py-2 px-3 cursor-pointer select-none"
                        style={teamLbSort === col.key ? { color: ndBlue } : undefined}
                ...
                <tbody>
                  {[...teamSchools].sort((a, b) => b[teamLbSort] - a[teamLbSort]).map(s => (
                    ...
                    >
                      <td className="py-3 px-3 text-sm text-[#0F1D2E]">
                        {s.shortName === selfSchool.shortName ? '→ ' : ''}{s.shortName || s.school}
                      </td>
                      <td className="py-3 px-3 text-xs text-[#7A8AA3]">{s.conference}</td>
...
function IPTab({ ipImpact }: { ipImpact: UCLAIPImpact }) {
  ...
  const withTotalEmv = estimateEmv(withData.likes * withData.contents, withData.comments * withData.contents);
    ...
    <div className="space-y-6">
      ...
      <GlassPanel className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] mb-2" style={{ color: ndBlue }}>IP Intelligence · Instagram Only</p>
            <p className="text-3xl md:text-4xl font-black text-[#0F1D2E] leading-tight">
              Posts with Notre Dame {active.label.toLowerCase()} drive{' '}
              <span style={{ color: liftPositive ? ndBlue : negative }}>
          ...
          <div className="flex flex-col items-start md:items-end gap-1">
            <span
              ...
              style={{ color: ndBlue, backgroundColor: 'rgba(12,35,64,0.08)', border: '1px solid rgba(12,35,64,0.3)' }}
            >
              Instagram only
            </span>
          </div>
        ...
        <div className="flex items-center gap-1 rounded-xl border border-[#E1E7F0] p-1 bg-[#F7F9FC]">
          {signals.map(s => (
            <button
              ...
              style={signal === s.id ? { backgroundColor: ndBlue } : undefined}
            >
              {s.label}
            </button>
          ))}
          ...
          {metrics.map(m => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                metric === m.id
                  ? 'text-[#0F1D2E] shadow-sm'
                  : 'text-[#5B6B82] hover:text-[#1E2A3B]'
              }`}
              style={metric === m.id ? { backgroundColor: ndGold } : undefined}
            >
              {m.label}
            </button>
          ))}
        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex items-baseline justify-between mb-1.5">
              <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: ndBlue }}>
            ...
            <div className="w-full bg-[#F0F4FA] rounded-lg h-9 overflow-hidden">
              <div
                ...
                style={{ backgroundColor: ndBlue, width: `${Math.max(withPct, 2)}%` }}
              />
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-[#E1E7F0] flex items-center gap-2">
            {liftPositive ? (
              <ArrowUpRight className="w-5 h-5" style={{ color: ndBlue }} />
            ) : (
              <ArrowDownRight className="w-5 h-5" style={{ color: negative }} />
            )}
            <span className="text-sm font-bold" style={{ color: liftPositive ? ndBlue : negative }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'EMV With', value: formatCurrency(withTotalEmv) },
        ].map(item => (
      ...
      <GlassPanel className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {signals.map(s => {
            ...
            const color = pos ? ndBlue : negative;
            ...
            return (
              <button
                ...
                style={
                  signal === s.id
                    ? { border: '1px solid rgba(12,35,64,0.4)', backgroundColor: 'rgba(12,35,64,0.05)' }
                ...
                <div className="space-y-1 text-xs text-[#5B6B82]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-[#0F1D2E]">
                      {formatCurrency(estimateEmv(s.data.yes.likes * s.data.yes.contents, s.data.yes.comments * s.data.yes.contents))}
                    </span>
                  </div>
                </div>
                {/* Mini ER bar */}
...
function buildIpImpactForDisplay(ipImpact: UCLAIPImpact | null, posts: UCLAAthletePost[]): UCLAIPImpact | null {
  if (!ipImpact) return null;

  const withCollab = posts.filter((post) => !!(post as UCLAAthletePost & { isCollaboration?: boolean }).isCollaboration);
  const withoutCollab = posts.filter((post) => !(post as UCLAAthletePost & { isCollaboration?: boolean }).isCollaboration);

  const bucketFromPosts = (items: UCLAAthletePost[]) => {
    const n = items.length || 1;
    const totalLikes = items.reduce((sum, post) => sum + (post.metrics?.likes || 0), 0);
    const totalComments = items.reduce((sum, post) => sum + (post.metrics?.comments || 0), 0);
    const avgEr = items.length
      ? items.reduce((sum, post) => sum + (post.metrics?.engagementRate || 0), 0) / items.length
      : 0;
    return {
      contents: items.length,
      likes: totalLikes / n,
      comments: totalComments / n,
      engagementRate: avgEr / 100,
      emv: estimateEmv(totalLikes, totalComments),
    };
  };

  const collabYes = bucketFromPosts(withCollab);
  const collabNo = bucketFromPosts(withoutCollab);
  const collabLift = collabNo.engagementRate > 0
    ? ((collabYes.engagementRate - collabNo.engagementRate) / collabNo.engagementRate) * 100
    : 0;
  ...
  return {
    ...
    collaboration: {
      yes: collabYes,
      no: collabNo,
      avgLift: Number(collabLift.toFixed(1)),
    },
    counts: {
      ...ipImpact.counts,
